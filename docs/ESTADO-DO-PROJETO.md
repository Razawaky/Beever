...
até aqui, era só desativação (DT-79). O outro lado é o agregado anônimo:
`conta.criada`, `conta.atualizada` e `consentimento.registrado` deixaram de
gravar apelido, e-mail e data de nascimento na trilha, que é imutável por
gatilho — se o dado pessoal entrasse nela, sobreviveria à exclusão para
sempre. Quem pode apagar: o próprio dono ou um administrador; o painel ganhou a
rota `POST /admin/usuarios/:id/definitivo` com botão na listagem. Seis testes
novos em `test/integration/exclusaoDefinitiva.test.js`, e **a E16 fecha
inteira**.

**Commit anterior:** T-16.1 — a composição de desktop das quatro telas mais
...
sessão no MySQL, contêiner e CI. Cada escolha fecha com tabela
decisão → alternativa → porquê, e o porquê da evolução sem reescrita.

**Commit anterior:** T-15.2 — os quatro diagramas do TCC
...
navegação inteira do painel administrativo: a classe era escrita elemento por elemento, e só
a landing tinha. Virou regra de base no `tema.css`, que vale inclusive para a tela que ainda
...
(DT-24) e o `normalizeEmail` saiu (DT-26). Laudo em `docs/14-VARREDURA-DE-SEGURANCA.md`.
975 testes passando.
**Commit anterior:** auditoria da E13, com as duas lacunas bloqueantes corrigidas na mesma
...
| O backup contra volume de dados real (T-14.6) | O dump de 175 KB é do banco de desenvolvimento, com 2 usuários e 37 itens. Tempo de dump, tempo de restauração e espaço em disco com dados de uso real seguem sem medição |
| O envio de ilustração dentro do contêiner (T-14.4) | O defeito corrigido era de permissão, e a permissão foi conferida na imagem que subiu: `/app/uploads` pertence a `node:node`, que é o usuário que roda a aplicação. O que **não** foi feito é o upload ponta a ponta lá dentro — exigiria login de administrador no contêiner, e a conferência parou na causa. O caminho em si tem teste desde a T-12.3, mas contra o disco da máquina, não contra o volume |
| Duas réplicas da aplicação ao mesmo tempo (RNF-38, T-14.4) | O compose sobe uma. Tirar o estado do contêiner foi o que a tarefa fez — sessão no MySQL, migrations num serviço à parte, uploads em volume — mas subir duas de fato, atrás de um balanceador, nunca foi exercitado. É a DT-115 |
| O portão do CI rodando de verdade (T-14.5) | O workflow tem teste estático que confere script, gatilho, variável obrigatória e a exceção do `.gitignore`, e ele passa. O que **não** aconteceu é uma execução real: nenhum pull request foi aberto, então sintaxe aceita pelo GitHub, disponibilidade das actions e o serviço de MySQL do runner seguem por provar. É a DT-117 |
| A cobertura como portão do pipeline (T-14.2) | `npm run test:cobertura` mede, reprova com código de saída 1 e foi conferido nos dois sentidos — piso alcançado sai zero, piso furado sai um. A T-14.5 fechou a parte que faltava: o job `cobertura` chama o comando a cada pull request, em execução separada da suíte, porque cobertura e cronômetro não se medem juntos |
| As telas de conquista e de liga em navegador real (T-13.4) | A marcação está coberta por teste pelo HTTP: o aviso do desbloqueio na Colmeia, o degrau travado mostrando alvo e progresso, a posição na liga e o ranque sem e-mail nem data de nascimento. O que **não** foi visto por olho humano é o desenho — as cinco escadas de quatro degraus em duas colunas a 320 px, a barra de progresso do degrau travado, a linha do próprio jogador destacada no ranque e o bloco de conquista na tela de resultado, que só aparece depois de uma partida de verdade. Vale o passe da DT-22 |
| O ícone da conquista (T-13.4) | `src/config/conquistas.js` é o ponto único de troca e hoje devolve emoji, um por família. `achievements.icon_path` continua nulo no banco e nenhuma tela o lê: quando a arte existir, ou o config aponta para os arquivos ou a coluna passa a ser a fonte, e essa escolha ainda não foi feita |
...
| T-13.4 Views | **feita** — `/conquistas` mostra as cinco escadas com o degrau travado exibindo alvo e progresso, e `/liga` mostra o ranque só por apelido, com a linha do próprio jogador marcada por fundo e pela palavra "você". O desbloqueio aparece no topo da Colmeia e no fim da partida. `conquistasDoJogador.js` junta os números das três fontes e avalia antes de desenhar, senão a tela mostraria "12 de 12 favos" num degrau travado. **A etapa fecha inteira** |
...
| ~~DT-117~~ | ~~O workflow é provado por teste estático, e nada disso executa o YAML~~ — **paga em 2026-09-04**: o pull request #1 rodou o portão pela primeira vez. Lint e build da imagem passaram; a suíte e a cobertura reprovaram, e o motivo foi exatamente o tipo de defeito que só o CI acha: `app.test.js` subia a aplicação sem criar banco de teste, então o `/health` respondia pelo banco de desenvolvimento da máquina de quem rodava e virou 503 no runner. A catraca de ramo, colada nos 93,11% desta máquina, também reprovou nos 92,82% do runner e desceu para 92. A execução seguinte saiu verde nos quatro jobs em 3m13s, e o portão passou a valer de verdade | T-14.5 | — |
...
| ~~DT-124~~ | ~~A regra escrita do Cofre do Tempo concorda no gênero errado: "rende 10% neste ${nomeDoCiclo}" vira "neste semana"~~ | T-15.5 | **Resolvida na T-17.1**: a frase passou a ser "rende 10% por ${nomeDoCiclo}", que funciona com qualquer nome que o banco traga, em vez de o código carregar tabela de gênero |
| ~~DT-125~~ | ~~Na rodada 1 do Cofre do Tempo a tabela do extrato desenha o cabeçalho sobre um corpo vazio, e o gráfico fica só com a linha da meta~~ | T-15.5 | **Resolvida na T-17.1**: gráfico e tabela nascem escondidos e aparecem no primeiro depósito; até lá a criança lê uma frase em vez de colunas vazias |
...
| ~~DT-26~~ | ~~`normalizeEmail()` remove pontos e sufixos `+` de endereços do Gmail: dois e-mails reais e distintos podem colidir no 409 de duplicado~~ — **resolvida na T-14.1**: saiu das três portas, ficou `trim` e minúsculas | auditoria da E03 | — |
...
| O limite global conta por sessão na leitura de quem está logado, e por endereço na escrita e para quem não entrou: a sala de aula sai de um IP só, e contá-la como uma pessoa reprovava a RNF-02 | auditoria da E14 |
| O portão de cobertura mede também quem decide a nota (`validadoresDeJogo`) e quem guarda a conta (`usersService`): service de cálculo é quem produz o número, mesmo quando o número só vira recompensa uma camada depois | auditoria da E14 |
| A página de privacidade só afirma o que o código faz hoje; promessa que depende de tarefa futura sai do texto e vira dívida com dono | auditoria da E14 |
...
### Sessão de 2026-08-27, T-13.2 e a conquista que se desbloqueia sozinha

Os quatro pontos onde os números mudam não custam a mesma coisa, e foi isso que
decidiu o desenho. Célula e favo já estão em mãos no fim da partida — o
`registrarConclusao` devolve `favoConcluido`, e contar as concluídas usa a UNIQUE
`(user_id, cell_id)` —, então eles são avaliados ali, e a conquista volta na
resposta do fechamento, porque a comemoração pertence à tela de resultado.
Patrimônio é o oposto: somar carteira, cofre e inventário é a conta mais cara do
sistema, e a Colmeia já a faz uma vez por visita para o cabeçalho. Refazê-la a cada
compra seria pagar caro no lugar errado.

O mel da conquista vem em lista própria, fora do que a partida rendeu. Somado, a
mesma célula pareceria pagar diferente das outras, e a criança perderia a conta.

**O teste de N+1 da Colmeia achou um erro meu antes de ele virar commit.** A
primeira versão perguntava em duas etapas — a escada, e depois quais o jogador já
tinha —, e a segunda pergunta só acontecia quando algum degrau era alcançado. O
número de consultas passava a depender do estado do jogador, e não do tamanho do
dado. O conserto foi na origem: `listarCriterioComEstado` traz a escada e o que ele
possui num `SELECT` só, o que deixou a contagem constante e ainda ficou mais barato
que a versão anterior.

Três testes existentes precisaram de ajuste, e nenhum foi afrouxado. Dois de
contagem de consultas ganharam uma visita de aquecimento a mais, porque desbloquear
paga mel, o mel move o patrimônio e o patrimônio pode destravar o degrau seguinte —
a medição precisa vir do estado estável. O terceiro, o de duplo clique na compra,
comparava contra um saldo absoluto e passou a comparar contra o saldo lido logo
antes: agora mede a compra, e não a compra somada ao bônus.

Duas dívidas nasceram desse mesmo encadeamento: a DT-104, porque a conquista de
patrimônio pode chegar uma visita depois de merecida, e a DT-105, porque o
patrimônio avaliado é o do momento e não o maior já alcançado — quem chega a cinco
...
### Sessão de 2026-08-31, T-13.4 e as telas que faltavam à E13

A conquista e a liga existiam inteiras no banco e nos services, e nenhuma das
duas tinha porta. Esta tarefa foi só tela, e a decisão de fundo foi mostrar o
degrau travado com alvo e progresso em vez de silhueta: a escada visível é o que
dá motivo para voltar amanhã, e é assim que os aplicativos do gênero fazem.

Os números da tela de conquistas vêm de três lugares diferentes — sequência,
progresso da trilha e patrimônio —, então nasceu o `conquistasDoJogador.js` em
vez de inchar o `achievementsService`, que só sabe de conquista. Ele avalia os
cinco critérios antes de ler o catálogo, senão a página mostraria "12 de 12
favos" num degrau ainda travado para quem chega sem passar pela Colmeia.

A liga mostra apelido, posição e pólen, e mais nada: o `listarMembrosComPolen`
já trazia `avatar_id`, e nem isso entrou, porque a RF-GAM-03 fala de ranking sem
dado pessoal. A linha do próprio jogador é destacada por fundo **e** pela palavra
"você", nunca só por cor. O teste confere que o e-mail e a data de nascimento não
aparecem no HTML.

O ícone ganhou `src/config/conquistas.js`, no mesmo desenho do catálogo do
mascote, e a DT-103 fica paga: a arte definitiva troca num arquivo só. A coluna
`achievements.icon_path` continua nula e sem leitor, e essa escolha fica para
quando a ilustração existir.
...
### Sessão de 2026-08-31, T-14.2 e a cobertura que ninguém tinha medido

O projeto tinha 975 testes e nenhuma medição: sabia-se que passavam, não se sabia
o que tocavam. A primeira medida mostrou 97,42% de linha e 86,30% de ramo nos
services — bom de partida, com o buraco exatamente onde mais importa, nos
caminhos de recusa dos services que mexem em mel.

A decisão de recorte foi não usar piso global. Piso de aplicação inteira deixa um
service de mel ficar em 60% com o total verde; o que a RNF-28 pede é 100% dos
services de cálculo, e é essa lista de vinte e quatro que mora em
`scripts/cobertura.js`, o único lugar onde ela existe. Linha fechou em 100% e não
se negocia. Ramo e função ficaram como catraca, e o laudo explica por quê: o que
sobra são reservas que nunca disparam, parâmetros com padrão e `catch` de falha de
infraestrutura, e teste escrito para perseguir esses três executa linha sem
afirmar nada.

Os testes novos valem mais do que o número. As guardas de saldo nunca tinham sido
exercitadas — são elas que impedem meio mel e mel negativo de chegarem ao livro.
A posse da partida também não: a suíte sempre jogou com o dono do token, e o que
impede alguém de fechar a partida de outra criança e creditar o mel na própria
conta estava escrito e não provado.

A medição achou oito exportações públicas sem um único chamador, quatro delas
invólucro de repository que o resto do código já chamava direto. Saíram.

E uma descoberta que muda como o CI da T-14.5 vai ser montado: cobertura e
cronômetro não se medem na mesma execução. A instrumentação infla o tempo e faz a
visita mais pesada reprovar a própria RNF-01. O `scripts/cobertura.js` marca a
execução e os testes de tempo se pulam sozinhos; quem cobra o teto continua sendo
o `npm run test:db`.
...
### Sessão de 2026-08-31, T-14.5 e o portão que não teria existido

A tarefa era escrever o workflow, e escrever o workflow foi a parte fácil. O que
quase passou batido é que o `.gitignore` ignorava `.github/` inteiro. A regra
tinha motivo — as skills, os agents e os hooks que moram ali são configuração
local de ferramenta de IA e não pertencem ao repositório —, mas ela também barra
o único diretório que o GitHub Actions lê. O arquivo existiria no disco, a tarefa
pareceria pronta, e nenhum runner jamais o executaria. A correção é uma exceção
cirúrgica: `.github/*` seguido de `!.github/workflows/`, que é a única forma que
funciona, porque o git não desce em diretório excluído por inteiro e a negação
sobre `.github/` não teria efeito nenhum.

O portão tem cinco jobs em paralelo. O `lint` roda ESLint e `npm audit` com
`--omit=dev --audit-level=high`, porque vulnerabilidade em dependência de
desenvolvimento não sobrevive ao `npm prune` do Dockerfile e travaria o projeto
por ferramenta que produção nem instala. O `testes` sobe MySQL 8.4 ao lado e usa
`test:db`, não `test`: com `TESTES_DE_BANCO=1` a ausência de banco vira falha em
vez de teste pulado em silêncio, que é a pior forma de portão. O `cobertura` roda
separado por medição da T-14.2, já que a instrumentação infla o tempo que a
RNF-01 cobra. O `imagem` constrói o Dockerfile e confere que a aplicação sobe. E
o `carga` mede trinta simultâneos só em `main`, com `continue-on-error`, porque
runner compartilhado é irregular demais para cronômetro reprovar merge.

A publicação da imagem em registro ficou combinada para depois do aceite, então a
RNF-40 está cumprida na primeira metade e aberta na segunda, como DT-116. E fica
a dívida honesta da tarefa, DT-117: `fluxoDeIntegracao.test.js` prova script,
gatilho, variável e a exceção do `.gitignore` em milissegundos, mas nada disso
executa o YAML — sintaxe aceita pelo GitHub e serviço de MySQL do runner só se
provam no primeiro pull request real. É a mesma lição da T-14.4, onde quatro
defeitos só apareceram ao construir a imagem de fato.
...
### Sessão de 2026-09-09, auditoria da E15 e as três correções que ela cobrou

A auditoria achou oito lacunas, e a primeira era bloqueante pelo motivo mais
constrangedor possível: `test/unit/tcc.test.js`, que guarda os documentos da
T-15.2, T-15.3 e T-15.4, **nunca tinha sido commitado**. Quinze testes verdes
existiam só no disco de uma máquina, e o portão de CI não rodava nenhum deles.
Metade das entregas da etapa estava sem a guarda que a própria etapa
estabeleceu como padrão. Entrou no repositório sem a constante morta que
quebrava o lint, e com uma guarda a mais.

A segunda lacuna foi encontrada por conferência e não por leitura: a figura ER
dizia ser "o mapa inteiro do banco em uma só visão" e mostrava 55 de 60
tabelas. As cinco ausentes eram `goal_plan_rules`, `goal_target_rules`,
`reward_modifiers`, `sessions` e `vault_transaction_types` — justamente as
tabelas de calibragem que as decisões travadas destacam como o motivo de não
haver valor mágico no código. Entraram na figura, e a guarda nova reprova
qualquer tabela de `migrations/` que fique de fora daqui em diante.

A terceira era uma contradição entre dois documentos da mesma etapa. O
`docs/23` afirmava sem ressalva que os controllers respondem JSON, enquanto a
matriz registra a RNF-39 como parcial: `GET /conquistas` e `GET /liga` só
renderizam HTML, o que foi conferido no `paginaController` antes de escrever.
Os dois pontos do documento passaram a dizer a exceção.

Cinco lacunas ficaram abertas, todas de risco médio ou baixo, e três viraram
dívida com número: o manual que não documenta sete variáveis de ambiente
(DT-127), o "184 requisitos" que na verdade são 186 (DT-128) e a senha demo
literal no script de evidências (DT-129). As outras duas são o manual não citar
`npm test` e a E15 não ter critérios de aceite escritos no roadmap, ao
contrário das etapas anteriores — a auditoria teve que inferir o aceite da
tabela de entregas.

O laudo está em `docs/27-AUDITORIA-DA-ETAPA-E15.md`, no molde das auditorias
...
conta; e a política de privacidade ainda não passou por revisão jurídica
(DT-130). O apagamento definitivo de conta, que na hora era só marcação de
inativo, foi a tarefa que encerrou a E16 depois — o relato está na sessão da
T-16.2 abaixo.
