...
| A cobertura como portão do pipeline (T-14.2) | `npm run test:cobertura` mede, reprova com código de saída 1 e foi conferido nos dois sentidos — piso alcançado sai zero, piso furado sai um. O que **não** existe é alguém chamando o comando: o portão do CI é a T-14.5, e até lá quem roda é quem lembra. Vale registrar junto que cobertura e cronômetro não se medem na mesma execução, então o pipeline vai precisar de dois comandos, não de um |
